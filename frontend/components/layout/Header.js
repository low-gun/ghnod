...
const MyPageMenuDrawerDynamic = dynamic(
  () => import("@/components/mypage/MyPageMenuDrawer"),
  { ssr: false, loading: () => null }
);
const MobileMenuDrawerDynamic = dynamic(
  () => import("@/components/common/MobileMenuDrawer"),
  { ssr: false, loading: () => null }
);
...
export default function Header({
  ...
  function renderCenterGroup() {
    ...
    return (
      <>
        <style>{`.header-menu::-webkit-scrollbar { display: none !important; }`}</style>
        <div
        ...
        >
          {centerGroup.map((item, idx) => (
            <div
              key={item.label || idx}
              style={{ position: "relative", minWidth: 0, maxWidth: "140px" }}
  ...
  return (
    <>
      ...
      {isCompactNav && showMobileMenu && (
        <MobileMenuDrawerDynamic
          open={showMobileMenu}
